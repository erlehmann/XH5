		<aside>
			...
			<?php if (!function_exists('dynamic_sidebar') || !dynamic_sidebar() ) : ?>
				...
				<li>
					<h2>XH5 Theme</h2>
					<p>XH5 ist ein Wordpress-Theme entwickelt mit XHTML5. XH5 schaut prima aus (außer im Internet-Explorer, aber wenn du den benutzt, kannst du diesen Text sowieso nicht lesen) und ist hoffentlich irgendwann einmal valides XHTML5 und CSS.</p>
				</li>
				<li>
				...
				<li>
					<?php if (is_404()) { ?>
					<p>Gib andere Begriffe in das Suchformular ein, vielleicht hast du dann mehr Erfolg.</p>

					<?php } elseif (is_category()) { ?>
					<p>Du schaust dir zur Zeit das Archiv für die Kategorie „<?php single_cat_title(''); ?>“ an.</p>

					<?php } elseif (is_day()) { ?>
					<p>Du schaust dir zur Zeit das Archiv für den Tag <?php the_time('l, F jS, Y'); ?> an.</p>

					<?php } elseif (is_month()) { ?>
					<p>Du schaust dir zur Zeit das Archiv für den Monat <?php the_time('F, Y'); ?> an.</p>

					<?php } elseif (is_year()) { ?>
					<p>Du schaust dir zur Zeit das Archiv für das Jahr <?php the_time('Y'); ?> an.</p>

					<?php } elseif (is_search()) { ?>
					<p>Du suchtest nach „<strong>'<?php the_search_query(); ?>'</strong>“.</p>

					<?php } elseif (isset($_GET['paged']) && !empty($_GET['paged'])) { ?>
					<p>Du schaust dir zur Zeit das Archiv von <a href="<?php echo bloginfo('url'); ?>/"><?php echo bloginfo('name'); ?></a> an.</p>
